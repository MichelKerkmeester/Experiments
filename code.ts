...
class SankeyLayout {
  ...
  constructor(data: FlowData[], settings: ChartSettings) {
    this.data = data;
    this.settings = settings;
  }

  // Calculate layout
  calculate(): { nodes: SankeyNode[]; flows: Flow[] } {
    this.assignLevels();
  ...
  private assignLevels(): void {
    ...
    this.data.forEach((flow) => {
      ...
      if (!nodeIncoming.has(flow.to)) nodeIncoming.set(flow.to, 0);
      nodeIncoming.set(flow.to, nodeIncoming.get(flow.to)! + 1);

      if (!nodeOutgoing.has(flow.from)) nodeOutgoing.set(flow.from, new Set());
  ...
  private calculateNodeSizes(): void {
    ...
    this.nodes.forEach((node, nodeId) => {
      const incomingTotal = node.incomingFlows.reduce(
        (sum, f) => sum + f.value,
        0
      );
      const outgoingTotal = node.outgoingFlows.reduce(
        (sum, f) => sum + f.value,
        0
      );
  ...
  private positionNodes(): void {
    ...
    const levelWidth = Math.max(
      100,
      (this.settings.width - this.settings.nodeWidth) / Math.max(1, maxLevel)
    );
    ...
    levels.forEach((levelNodes, level) => {
      ...
      const totalHeight = levelNodes.reduce(
      ...
      let currentY = Math.max(
        padding,
        (this.settings.height - totalHeight - totalSpacing) / 2
      );
  ...
  private calculateFlowPositions(): void {
    this.nodes.forEach((node) => {
  ...
  private generateFlows(): void {
    const flowPositions = new Map<
      string,
      { outgoingY: number; incomingY: number }
    >();
    ...
    this.flows = this.data.map((flowData) => {
      ...
      const thickness = Math.max(
        2,
        Math.min(30, (flowData.value / maxValue) * 25)
      );
  ...
  private getFlowColor(fromId: string, toId: string): string {
...
function createSankeyChart(data: FlowData[], settings: ChartSettings): void {
  // Create main frame
  ...
  flows.forEach((flow, index) => {
    try {
      ...
      const topPath = `M ${fromX} ${
        fromY - flow.thickness / 2
      } C ${controlPoint1X} ${fromY - flow.thickness / 2} ${controlPoint2X} ${
        toY - flow.thickness / 2
      } ${toX} ${toY - flow.thickness / 2}`;
      const bottomPath = `L ${toX} ${
        toY + flow.thickness / 2
      } C ${controlPoint2X} ${toY + flow.thickness / 2} ${controlPoint1X} ${
        fromY + flow.thickness / 2
      } ${fromX} ${fromY + flow.thickness / 2}`;
  ...
  nodes.forEach((node) => {
    ...
    if (settings.enableGradients) {
      const nodeGradient: GradientPaint = {
        type: "GRADIENT_LINEAR",
        gradientTransform: [
          [1, 0, 0],
          [0, 1, 0],
...
figma.ui.onmessage = (msg) => {
  ...
  if (msg.type === "clear-canvas") {
    ...
    const sankeyFrames = figma.currentPage.children.filter(
      (node) => node.type === "FRAME" && node.name.includes("Sankey Chart")
    );
